...
def generaldealerexportation(listofdealersurls: list,listofdealersregions: list,distinguisher:str) ->None:
    import pandas as pd
    nameforfile=f'GeneralListofDealers_{distinguisher}.csv'
    headerforcsv=['No.','State/Province/Region','URLs']
    df=pd.DataFrame(columns=headerforcsv)

    for index in range(len(listofdealersurls)):
       
        df.loc[index]=[index+1,listofdealersregions[index], listofdealersurls[index]]
...
def counterdeprogreso(counter,total):
        urlbetweenpauses=10 # ;'Number of urls before every stop'
        if (counter != 0) and (counter % urlbetweenpauses==0):
            time.sleep(5)
        print('\n')
...
def pagesrequests():
    ...
    req = Request(
        url='https://www.henryusa.com/own-a-henry/find-a-henry-dealer/', 
        headers=getHeaders()
    )
    page = urlopen(req)
    soup = BeautifulSoup(page,"html.parser")
...
def promptinguser_startprogram():
    ...
    if startcondition=="1":
        ...
        'for the US'
        generaldealerexportation(listofdealersurlsUS,listofdealersTitlesUS,'US')
        'for Canada'
        generaldealerexportation(listofdealersurlsCanada,listofdealersTitlesCanada,'CA')
